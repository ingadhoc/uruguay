...
class TestUyEdiSpecialRegime(TestUyEdi):
    ...
    def test_85_check_move_scope(self):
        """The block only applies to electronic sale documents that will become a CFE:
        vendor bills (purchase VAT) and export CFEs must not be blocked at post."""
        purchase_tax_22 = self.env["account.tax"].search(
            [("type_tax_use", "=", "purchase"), ("amount", "=", 22), ("company_id", "=", self.company_uy.id)],
            limit=1,
        )
        bill = self.env["account.move"].create(
            {
                ...
                "l10n_latam_document_number": "A0000099",
                "invoice_line_ids": [
                    Command.create(
                        {
                            "product_id": self.service_exempt.id,
                            "quantity": 1.0,
                            "price_unit": 100.0,
                            "tax_ids": [(6, 0, purchase_tax_22.ids)],
                        }
                    )
                ],
            }
        ...
        bill.action_post()
        self.assertEqual(bill.state, "posted", "Vendor bills must not be blocked by the special regime check")

        expo_invoice = self._create_move(
            l10n_latam_document_type_id=self.env.ref("l10n_uy.dc_e_inv_exp").id,
            partner_id=self.foreign_partner.id,
            invoice_incoterm_id=self.env.ref("account.incoterm_FOB").id,
            l10n_uy_edi_cfe_sale_mode="1",
            ...
            invoice_line_ids=[
                Command.create(
                    {
                        "product_id": self.product_vat_22.id,
                        "price_unit": 100.0,
        ...
        # Assert on the helper instead of posting: with l10n_uy_ux installed, posting sends the
        # CFE and the NATIVE expo check ("Export CFE can only have 0% vat taxes") reverts it to
        # draft, which is unrelated to what this test verifies (this module must not block it)
        self.assertFalse(
            expo_invoice._l10n_uy_edi_special_regime_taxed_lines_error(),
            "Export CFEs keep the standard behavior: the special regime check must not apply to them",
        )
