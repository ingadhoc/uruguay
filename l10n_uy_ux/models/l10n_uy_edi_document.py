...
class L10nUyEdiDocument(models.Model):
    ...
    def _get_partner_from_xml(self, xml_tree, partner_vat_RUC):
        ...
        partner = self.env["res.partner"]._retrieve_partner(vat=partner_vat_RUC, company=move.company_id)
        ...
        cfe_currency = self.env['res.currency'].with_context(active_test=False).search(
            [('name', '=', xml_tree.findtext(".//{*}TpoMoneda"))], limit=1)
        if partner and partner.property_account_payable_id.currency_id and partner.property_account_payable_id.currency_id != cfe_currency:
            domain = [('vat', '=', partner_vat_RUC),
                    *self.env['res.partner']._check_company_domain(company or self.env.company), ('company_id', '!=', False)]
            # Search with same currency
            partner = self.env['res.partner'].search(domain + [('property_account_payable_id.currency_id', '=', cfe_currency.id)])
            # Seearch without currency
            partner = partner or self.env['res.partner'].search(domain + ['|',  ('property_account_payable_id', '=', False), ('property_account_payable_id.currency_id', '=', False)])
