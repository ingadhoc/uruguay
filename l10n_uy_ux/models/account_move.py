...
class AccountMove(models.Model):
    ...
    def action_switch_move_type(self):
        ...
        super(AccountMove, self.with_context(switch_move_type=new_move_type)).action_switch_move_type()
    ...
    def _compute_l10n_latam_document_type(self):
        ...
        if uy_einvoices := self.filtered(
            ...
            uy_credit_notes = uy_einvoices.filtered(lambda m: m.reversed_entry_id)
            if new_move_type and "refund" in new_move_type:
                uy_credit_notes |= uy_einvoices
            if uy_credit_notes:
